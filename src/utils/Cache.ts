...
class Cache implements CacheInterface {
    ...
    constructor() {
        // @ts-ignore
        this.cacheClient = redis.createClient(process.env.REDIS_PORT, process.env.REDIS_HOST);
    }

    sadd(key: string, values: Array<string>, timeExpiration: number) {
        return new Promise((resolve, reject) => {
            this.cacheClient.sadd(key, values, async (error) => {
                if (error) {
                    reject(error);
                    return;
                }
    ...
    setExpirationTimeInKey(key: string, timeExpiration: number) {
        return new Promise((resolve, reject) => {
            this.cacheClient.expire(key, timeExpiration, (error) => {
                if (error) {
                    reject(error);
                    return;
                }
    ...
    set(key: string, values: string, timeExpiration: number = 10) {
        return new Promise((resolve, reject) => {
            this.cacheClient.set(key, values, async (error) => {
                if (error) {
                    reject(error);
                    return;
                }
